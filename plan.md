...
## 진행 중 — 월별 뷰에 취소된 예약 표시

> 요청: "취소된 예약, 월별에서는 보이게 해줘"

### 현황
- `Month.tsx:49`에서 `status !== 'cancelled'` 조건으로 취소 예약만 월별 그리드에서 제외 중.
- 노쇼(`noshow`)는 이미 표시되며, `ReservationList`가 취소/노쇼를 `$inactive`(회색조+투명도 0.5) 스타일로 렌더(`ReservationList.tsx:53,118`).

### 수정
- `Month.tsx`의 데이터 필터에서 `&& reservation.status !== 'cancelled'` 제거 → 취소 예약도 노쇼와 동일하게 비활성 스타일로 월별에 노출. "전체(N)" 카운트도 자동 포함.
- 범위: 월별 뷰만(Timeline 등 다른 뷰의 취소 숨김은 요청 외라 유지).

### 리스크
- 취소 예약이 많은 날 그리드가 길어질 수 있으나 비활성 스타일로 구분됨.
